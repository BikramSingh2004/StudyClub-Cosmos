...
function handleRoomTransition(socketId) {
  ...
  if (newRoomId) {
    ...
    Message.find({ roomId: "room_" + newRoomId })
      ...
      .lean()
      .then((msgs) => {
        io.to(socketId).emit("room:chat-history", {
          roomId: newRoomId,
...
function checkSpatialProximity(socketId) {
  ...
  for (const otherSid of roomSet) {
    ...
    if (dist < PROXIMITY_RADIUS) {
      ...
      io.to(socketId).emit("proximity:near", {
        peerId: otherSid,
        username: other.username,
        avatarColor: other.avatarColor,
        distance: dist,
      });
      io.to(otherSid).emit("proximity:near", {
        peerId: socketId,
        username: player.username,
        avatarColor: player.avatarColor,
        distance: dist,
...
io.on("connection", (socket) => {
  ...
  socket.on("room:chat-history-request", async ({ roomId }) => {
    if (!roomId) return;

    try {
      const msgs = await Message.find({ roomId: "room_" + roomId })
        .sort({ timestamp: -1 })
        .limit(50)
        .lean();
